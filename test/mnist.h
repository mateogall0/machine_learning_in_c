...
#define NUM_BINARY_SAMPLES 1000 // Define the number of samples in the binary subset
double train_image_binary[NUM_BINARY_SAMPLES][SIZE];
int train_label_binary[NUM_BINARY_SAMPLES];
void create_binary_subset() {
    int i, j, binary_index = 0;

    // Loop through the training set
    for (i = 0; i < NUM_TRAIN; i++) {
        if (train_label[i] == 0 || train_label[i] == 1) {
            // Copy the image and label to the binary subset
            for (j = 0; j < SIZE; j++) {
                train_image_binary[binary_index][j] = train_image[i][j];
            }
            train_label_binary[binary_index] = train_label[i];
            binary_index++;
            if (binary_index == NUM_BINARY_SAMPLES) // Stop when the desired number of samples is reached
                break;
        }
    }
}

void print_binary_subset() {
    int i, j;

    for (i = 0; i < NUM_BINARY_SAMPLES; i++) {
        printf("Image %d:\n", i + 1);
        for (j = 0; j < SIZE; j++) {
            printf("%1.1f ", train_image_binary[i][j]);
            if ((j + 1) % 28 == 0) {
                printf("\n");
            }
        }
        printf("Label: %d\n\n", train_label_binary[i]);
    }
}